...
class DatabaseService {
  ...
  Future<void> addGroup(String storeId, String name, int color) async {
    try {
      final groupsRef = _firestore.collection('stores').doc(storeId).collection('groups');
      final snapshot = await groupsRef.orderBy('order', descending: true).limit(1).get();
      int nextOrder = 0;
      if (snapshot.docs.isNotEmpty) {
        nextOrder = ((snapshot.docs.first.data()['order'] as num?)?.toInt() ?? 0) + 1;
      }
      await groupsRef.add({
        'name': name,
        'color': color,
      ...
      await GlobalErrorHandler.recordError(e, stack);
      throw DatabaseException('خطأ في إضافة القسم: $e', error: e);
    }
  }
  ...
  Future<void> updateGroup(String storeId, String groupId, String newName) async {
    try {
      ...
      await GlobalErrorHandler.recordError(e, stack);
      throw DatabaseException('خطأ في تعديل القسم: $e', error: e);
    }
  }

  Future<void> addProduct(String storeId, String groupId, Map<String, dynamic> data) async {
    try {
      await _firestore
          .collection('stores')
          .doc(storeId)
      ...
      await GlobalErrorHandler.recordError(e, stack);
      throw DatabaseException('خطأ في إضافة المنتج: $e', error: e);
    }
  }

  Future<void> updateProduct(String storeId, String groupId, String productId, Map<String, dynamic> data) async {
    try {
      await _firestore
          .collection('stores')
          .doc(storeId)
      ...
      await GlobalErrorHandler.recordError(e, stack);
      throw DatabaseException('خطأ في تعديل المنتج: $e', error: e);
    }
  }
}
...
// ============================================================================
// PART 7: AUTO BACKUP SERVICE (NEW)
// ============================================================================
...
class AutoBackupService {
  ...
  static Future<void> checkAndBackup() async {
    try {
      ...
      final currentVersion = '1.0.0'; // عند كل تحديث، قم بتغيير هذا الرقم
      final savedVersion = prefs.getString(kVersionKey);

      if (savedVersion != currentVersion) {
        print('🔄 اكتشاف تحديث جديد: $savedVersion → $currentVersion');
        await _performFullBackup();
        await prefs.setString(kVersionKey, currentVersion);
        print('✅ تم حفظ النسخة الجديدة: $currentVersion');
      } else {
        print('ℹ️ لا يوجد تحديث جديد، تخطي النسخ الاحتياطي');
      }
    } catch (e) {
      print('⚠️ فشل النسخ الاحتياطي التلقائي: $e');
    }
  }
...
void main() async {
  ...
  // تشغيل النسخ الاحتياطي التلقائي قبل التحديث
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetMaterialApp(
      ...
      theme: ThemeData(
        useMaterial3: true,
        scaffoldBackgroundColor: const Color(0xFF7BB3D9), // خلفية التطبيق بالكامل
...
// ============================================================================
// PART 10: CUSTOM BACKGROUND (Tarweeqa Style)
// ============================================================================
